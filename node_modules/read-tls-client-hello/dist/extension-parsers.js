...
exports.extensionParsers = void 0;
exports.parseSniExtension = parseSniExtension;
exports.parseSupportedGroupsExtension = parseSupportedGroupsExtension;
exports.parseEcPointFormatsExtension = parseEcPointFormatsExtension;
exports.parseSignatureAlgorithmsExtension = parseSignatureAlgorithmsExtension;
exports.parseAlpnExtension = parseAlpnExtension;
exports.parseMaxFragmentLengthExtension = parseMaxFragmentLengthExtension;
exports.parseStatusRequestExtension = parseStatusRequestExtension;
exports.parseStatusRequestV2Extension = parseStatusRequestV2Extension;
exports.parsePaddingExtension = parsePaddingExtension;
exports.parseCompressCertificateExtension = parseCompressCertificateExtension;
exports.parseRecordSizeLimitExtension = parseRecordSizeLimitExtension;
exports.parseSessionTicketExtension = parseSessionTicketExtension;
exports.parsePreSharedKeyExtension = parsePreSharedKeyExtension;
exports.parseSupportedVersionsExtension = parseSupportedVersionsExtension;
exports.parseCookieExtension = parseCookieExtension;
exports.parsePskKeyExchangeModesExtension = parsePskKeyExchangeModesExtension;
exports.parseSignatureAlgorithmsCertExtension = parseSignatureAlgorithmsCertExtension;
exports.parseKeyShareExtension = parseKeyShareExtension;
exports.parseRenegotiationInfoExtension = parseRenegotiationInfoExtension;
exports.parseHeartbeatExtension = parseHeartbeatExtension;
exports.parseEncryptedClientHelloExtension = parseEncryptedClientHelloExtension;
exports.parseApplicationSettingsExtension = parseApplicationSettingsExtension;
// --- Parsers for extensions already extracted in the main module ---
function parseSniExtension(data) {
...
function parseSupportedGroupsExtension(data) {
...
function parseEcPointFormatsExtension(data) {
    const length = data[0];
    const formats = [];
...
function parseMaxFragmentLengthExtension(data) {
    return { maxFragmentLength: data[0] };
}
function parseStatusRequestExtension(data) {
    return { statusType: data[0] };
}
function parseStatusRequestV2Extension(data) {
    const statusTypes = [];
...
function parsePaddingExtension(data) {
    return { paddingLength: data.byteLength };
}
function parseCompressCertificateExtension(data) {
    const length = data[0];
    const algorithms = [];
...
function parseRecordSizeLimitExtension(data) {
    return { recordSizeLimit: data.readUInt16BE(0) };
}
function parseSessionTicketExtension(data) {
...
function parseSupportedVersionsExtension(data) {
    const length = data[0];
    const versions = [];
    for (let i = 1; i < 1 + length; i += 2) {
...
function parseCookieExtension(data) {
    const cookieLength = data.readUInt16BE(0);
    return { cookieLength };
}
function parsePskKeyExchangeModesExtension(data) {
    const length = data[0];
    const modes = [];
    for (let i = 1; i <= length; i++) {
        modes.push(data[i]);
...
function parseSignatureAlgorithmsCertExtension(data) {
    // Same format as signature_algorithms
    return parseSignatureAlgorithmsExtension(data);
}
function parseKeyShareExtension(data) {
    const entries = [];
    const totalLength = data.readUInt16BE(0);
    let offset = 2;
    const end = 2 + totalLength;
...
function parseRenegotiationInfoExtension(data) {
    const renegotiatedConnectionLength = data[0];
    return { renegotiatedConnectionLength };
}
function parseHeartbeatExtension(data) {
    return { mode: data[0] };
}
function parseEncryptedClientHelloExtension(data) {
    const type = data[0];
    if (type === 1) {
    ...
    // Outer
    let offset = 1;
    const kdfId = data.readUInt16BE(offset);
    offset += 2;
    const aeadId = data.readUInt16BE(offset);
    offset += 2;
    const configId = data[offset];
    offset += 1;
    const encLength = data.readUInt16BE(offset);
    offset += 2;
...
function parseApplicationSettingsExtension(data) {
    // Same format as ALPN
...
//# sourceMappingURL=extension-parsers.js.map